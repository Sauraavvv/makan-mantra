"""
Update SEO descriptions in the MongoDB `location_pages` collection from slug_content_results.json.

Usage:
  python3 update_slug_content_results_mongo.py
  python3 update_slug_content_results_mongo.py --apply

Behavior:
  - Reads MONGODB_URL from backend/.env (or env var).
  - Loads slug_content_results.json from the workspace root.
  - Matches existing location_pages documents by slug, after pluralizing the
    property-type segment of the JSON's slug (e.g. "shop-for-sale-in-mumbai"
    -> "shops-for-sale-in-mumbai") to match the DB's actual city-page slugs.
  - Updates only meta_description and on_page_description (source fields:
    metaDescription and onpageDescription), preserving existing document shape
    (top-level vs nested under seo). Also bumps updated_at to the current
    UTC time on every matched document.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Update location_pages meta_description and on_page_description by slug."
    )
    parser.add_argument(
        "--json-path",
        type=Path,
...
def pluralize_slug(slug: str, property_type: str) -> str:
    """Rewrite the leading property-type segment of `slug` to its plural form.

    The JSON's slug/target_slug fields use a singular property type (e.g.
    "shop-for-sale-in-mumbai"), but the corresponding location_pages city
    documents use the plural form (e.g. "shops-for-sale-in-mumbai").
    """
    prefix = property_type.strip().lower().replace(" ", "-")
    if not slug.startswith(prefix + "-"):
        return slug
    return prefix + "s" + slug[len(prefix):]
...
def load_records(json_path: Path) -> list[dict[str, Any]]:
    ...
    for index, doc in enumerate(docs, start=1):
        ...
        property_type = doc.get("property_type") if isinstance(doc, dict) else None
        meta_description = doc.get("metaDescription") if isinstance(doc, dict) else None
        on_page_description = doc.get("onpageDescription") if isinstance(doc, dict) else None
        ...
        match_slug = pluralize_slug(slug, property_type)

        if match_slug in seen_slugs:
            print(f"WARNING: Duplicate slug in JSON, skipping: {match_slug}")
            continue
...
async def fetch_existing_docs(col: Any, slugs: list[str]) -> dict[str, dict[str, Any]]:
    # Scoped to location_category=city: these are the only documents this
    # JSON's content is meant for. This also guards against a slug collision
    # like Chandigarh, where the plural form of a JSON slug happens to match
    # an unrelated state/district document instead of the intended city page.
    ...
    for slug_chunk in chunked(slugs, CHUNK_SIZE):
        cursor = col.find(
            {"slug": {"$in": slug_chunk}, "location_category": "city"}, projection
        )
...
def resolve_match_slug(record: dict[str, Any], existing_by_slug: dict[str, Any]) -> str:
    """Prefer the pluralized slug; fall back to the JSON's raw slug for the
    rare city page (e.g. Chandigarh) that never got pluralized in the DB."""
    if record["slug"] in existing_by_slug:
        return record["slug"]
    if record["source_slug"] in existing_by_slug:
        return record["source_slug"]
    return record["slug"]


async def update_location_pages(args: argparse.Namespace) -> None:
    ...
    try:
        candidate_slugs = [record["slug"] for record in records] + [
            record["source_slug"] for record in records
        ]
        existing_by_slug = await fetch_existing_docs(col, candidate_slugs)
        ...
        for record in records:
            match_slug = resolve_match_slug(record, existing_by_slug)
            existing_doc = existing_by_slug.get(match_slug)
            if existing_doc is None:
                missing_records.append(record["slug"])
                continue

            payload = build_set_payload(record, existing_doc, args.field_mode)
            ...
            operations.append(UpdateOne({"slug": match_slug}, {"$set": payload}))

        missing_slugs = missing_records
        print(f"Matched slugs in DB: {len(records) - len(missing_slugs)}")
        ...
        if missing_slugs:
            ...
            for slug in missing_slugs[:50]:
                print(f"  - {slug}")
            if len(missing_slugs) > 50:
                print(f"  ... and {len(missing_slugs) - 50} more")
    finally:
        client.close()


if __name__ == "__main__":
    asyncio.run(update_location_pages(parse_args()))
