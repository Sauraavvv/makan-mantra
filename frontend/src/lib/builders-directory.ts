 ...
 * hand.
 */
const BUILDER_LOGOS = builderLogos as Record<string, { id: string; tile?: "dark" }>;

/**
 * Logos are drawn at 56–64px, so 256 wide covers a 4x screen. `c_limit` never
  ...
  for (const [state, records] of Object.entries(BUILDERS_BY_STATE)) {
    for (const record of records) {
      ...
      builders.push({
        ...
        logo: logoEntry && cldUrl(logoEntry.id, LOGO_TRANSFORM),
