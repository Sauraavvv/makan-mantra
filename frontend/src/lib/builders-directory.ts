import builderLogos from "@/data/builder-logos.json";
...
export type DirectoryBuilder = {
  ...
  id: string;
  /**
   * The builder half of the id, on its own — what `?builder=` carries when the
   * home page links straight to this card's profile. Not unique by itself: it
   * takes `?state=` alongside it to name one card.
   */
  slug: string;
  ...
  projects: string[];
  cities: string[];
  /** Cloudinary delivery URL; absent for a builder with no logo on file. */
  logo?: string;
  /**
   * Which tile the logo has to sit on. A white-ink logo on a transparent
   * background disappears on a white tile — Ashiana's wordmark left nothing but
   * the red mark — so those are marked `"dark"` in the data.
   */
  logoTile?: "light" | "dark";
  href?: string;
};
...
/**
 * Registered name → Cloudinary `public_id`, keyed on the exact `builder_name`
 * spelling used in `state-builders.json` so the lookup needs no fuzzy matching.
 * Two entries can share an asset where the data spells one company two ways
 * ("Ashiana Housing Limited" / "Ashiana Housing Ltd.").
 *
 * Regenerated by `scripts/upload-builder-logos.mjs`; nothing here is edited by
 * hand.
 */
const BUILDER_LOGOS = builderLogos as Record<
  string,
  { id: string; tile?: "dark"; zoom?: number }
>;

/**
 * Logos are drawn at 80px, so 256 wide covers a 3x screen. `c_limit` never
 * upscales a small source, and `f_auto` keeps the transparency the PNGs rely on.
 */
const LOGO_TRANSFORM = "w_256,c_limit,q_auto,f_auto";

/**
 * A few logos were saved with their frame still on — Raheja's carries a hairline
 * along its top edge. `zoom` crops that fraction of the image from the centre
 * before it is sized, which trims the edge away without touching the upload, so
 * the number can be tuned or dropped at any time.
 */
function logoTransform(zoom?: number) {
  return zoom ? `c_crop,w_${zoom},h_${zoom},g_center/${LOGO_TRANSFORM}` : LOGO_TRANSFORM;
}
...
function buildDirectory(): DirectoryBuilder[] {
  ...
  for (const [state, records] of Object.entries(BUILDERS_BY_STATE)) {
    for (const record of records) {
      ...
      const logoEntry = BUILDER_LOGOS[record.builder_name];

      const slug = stateSlug(record.builder_name);
      ...
      builders.push({
        ...
        logo: logoEntry && cldUrl(logoEntry.id, logoTransform(logoEntry.zoom)),
        logoTile: logoEntry?.tile ?? "light",
      });
    }
  }
